    ...
    for packets in packets_per_core_list:
        ...
        for load in sorted(set(load_percents)):
            ...
            if active_cores in tested_core_counts:
                print(f"\nSkipping {load}% Load on {label} (Already tested {active_cores} physical cores)")
            ...
            print(f"\nTesting: {label} Grid | {packets} Pkts/Core | {load}% Load ({active_cores}/{total_cores} Cores awake)")
